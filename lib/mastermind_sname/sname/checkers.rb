module MastermindSname
  class Logics
    ...
    def self.get_no_correct_elements
      no_of_elements = 0
      @colour_code.each do |val|
        no_of_elements += 1 if @user_guess.split("").include? val
      end
      no_of_elements
    end
    ...
    def self.get_no_correct_positions
      ...
      i = 0
      @colour_code.each do |val|
        correct_positions += 1 if val == @user_guess[i]
        i += 1
      end
      correct_positions
    end
    ...
    def self.get_feedback(user_guess, colour_code)
      ...
      feedback = "#{@user_guess} has #{get_no_correct_elements} of"\
